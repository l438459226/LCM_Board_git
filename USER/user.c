...
//BYTE buffer[10805]; // file copy buffer	10239
BYTE buffer1[513];
char fb_buffer[256];
...
//	unsigned char *lp;
	...
	srambuf = mymalloc(SRAMIN,12288);	//12k
	if(srambuf == NULL) {printf(" malloc error!:15");myfree(SRAMIN,srambuf);return ; }

    res = f_read(&fsrc, srambuf, 0x36, &br); //???????
	bmphead = (BITMAPINFO *)srambuf;
	...
	if(bmphead->bmfHeader.bfType != 0x4D42) {myfree(SRAMIN,srambuf);return ;}//??? MB
	if(Data_Offset !=54 ) {myfree(SRAMIN,srambuf);return ;} ;
	...
	for (;;) 
	{
		if((porter)>=10800)	{
			...
			res = f_read(&fsrc, srambuf, 10800, &br);
			ll = 10800;
		}
		else {
			ll = porter-Data_Offset;
			if(ll<=0) {myfree(SRAMIN,srambuf);return ;}
			res = f_lseek(&fsrc, Data_Offset);
			res = f_read(&fsrc, srambuf, porter-Data_Offset, &br);
			porter = 0;
		}
		 for(;;)
			{
				if(ll>(Lcd_LCDH*3)){		//size
					...
					{
						B=(uint8_t)( srambuf[ll+a]);a++;		  	//BRG?RGB			
						G=(uint8_t)( srambuf[ll+a]);a++;	
						R=(uint8_t)( srambuf[ll+a]);a++;	
					...
					for(a = 0; a <ll; )
					{
						B=(uint8_t)( srambuf[a]);a++;		  	//BRG?RGB			
						G=(uint8_t)( srambuf[a]);a++;	
						R=(uint8_t)( srambuf[a]);a++;	
						LcdDrvWriteData(R);	LcdDrvWriteData(G);LcdDrvWriteData(B);
					}	
					break;
...
//	unsigned char *lp;
	...
	srambuf = mymalloc(SRAMIN,12288);	//12k
	if(srambuf == NULL) {printf("14:malloc error!");myfree(SRAMIN,srambuf);return; }
  ...
  Data_Offset = bmphead->bmfHeader.bfOffBits;//+ buffer[11]<<8+ buffer[12]<<16+ buffer[13]<<24;
	...
	for (;;) 
	{
		...
		for(a = 10799;; )
			{
				R=(uint8_t)( srambuf[a--]);if(a==0) break;	  	//BRG?RGB			
				G=(uint8_t)( srambuf[a--]);if(a==0) break;
				B=(uint8_t)( srambuf[a--]);if(a==0) break;
				LcdDrvWriteData(R);	LcdDrvWriteData(G);LcdDrvWriteData(B);
			}	
			R=(uint8_t)( srambuf[2]);	  	//BRG?RGB		
			G=(uint8_t)( srambuf[1]);	
			B=(uint8_t)( srambuf[0]);	
			LcdDrvWriteData(R);	LcdDrvWriteData(G);LcdDrvWriteData(B);
...
{ 
	...
	srambuf = mymalloc(SRAMIN,10240);
	if(srambuf == NULL) {printf("13:malloc error!");myfree(SRAMIN,srambuf);return; }
	...
	for (;;) 
	{
		res = f_read(&fsrc, srambuf, (10240-1), &br);
    	for(a = 0; a <(10240-1); )
		{
			B=(uint8_t)( srambuf[a]);a++;	  	//BRG?RGB		
			G=(uint8_t)( srambuf[a]);a++;	
